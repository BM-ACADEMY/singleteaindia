...
const FranchisesList = () => {
  ...
  const handleDelete = async () => {
    try {
      ...
      setFranchises(franchises.filter((franchise) => franchise._id !== deletingFranchise._id));
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      const response = await axiosInstance.put(`/franchises/${editingFranchise._id}`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      setFranchises(franchises.map((f) => (f._id === editingFranchise._id ? response.data : f)));
      setEditingFranchise(null);
      setNewImages([]);
      // Clean up image previews
      newImages.forEach((image) => URL.revokeObjectURL(URL.createObjectURL(image)));
  ...
  return (
    <div className="p-4 sm:p-6 min-h-screen bg-gray-50">
      ...
      {error && (
        <p className="text-red-500 bg-red-50 p-3 rounded-lg mb-6 text-sm">{error}</p>
      )}
      ...
      {editingFranchise && (
        <div className="fixed inset-0 bg-black/70 flex justify-center items-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-5xl h-[90vh] overflow-y-auto p-6 border border-gray-100 animate-fade-in relative">
            <button
              onClick={() => {
                setEditingFranchise(null);
                newImages.forEach((image) => URL.revokeObjectURL(URL.createObjectURL(image)));
            ...
            <form onSubmit={handleUpdate} className="space-y-4 sm:space-y-5">
              ...
              <div className="flex justify-end gap-2 sm:gap-3">
                ...
                <button
                  type="submit"
                  className="px-3 sm:px-5 py-2 sm:py-2.5 rounded-lg bg-violet-600 text-white hover:bg-violet-700 font-medium text-sm transition-all duration-200 active:scale-95 disabled:opacity-50"
                  disabled={loading}
                >
                  {loading ? "Saving..." : "Save"}
                </button>
        ...
        <div className="fixed inset-0 bg-black/70 flex justify-center items-center z-50 p-4">
          <div className="flex flex-col items-center bg-white shadow-2xl rounded-xl py-4 sm:py-6 px-4 sm:px-5 w-full max-w-full sm:max-w-md border border-gray-100 animate-fade-in">
            ...
            <p className="text-sm text-gray-600 mt-2 text-center px-2">
              Do you really want to delete <b>{deletingFranchise.title}</b>? <br />
            ...
            <div className="flex items-center justify-center gap-3 sm:gap-4 mt-4 sm:mt-5 w-full">
              ...
              <button
                ...
                className="w-full sm:w-32 h-9 sm:h-10 rounded-lg text-white bg-red-600 font-medium text-sm hover:bg-red-700 active:scale-95 transition duration-200 disabled:opacity-50"
                disabled={loading}
              >
